...
RegisterNetEvent("paybills")
AddEventHandler("paybills",function()
...
RegisterNetEvent("es_extended:changename")
AddEventHandler("es_extended:changename",function()
	ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'firstname', {
		title = "이름변경(firstname)",
	}, function (data, menu)
		local firstname = data.value
		if firstname == nil then
			ESX.ShowNotification('invalid name!')
		else
			TriggerServerEvent("es_extended:changefirstname", firstname)
			menu.close()
		end
	...
	ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'lastname', {
		title = "이름변경(lastname)",
	}, function (data, menu)
		local lastname = data.value
		if lastname == nil then
			ESX.ShowNotification('invalid name!')
		else
			TriggerServerEvent("es_extended:changelastname", lastname)
			menu.close()
		end
	end, function (data, menu)
		menu.close()
...
AddEventHandler('esx:playerLoaded', function(playerData)
	...
	if Config.EnableHud then
		print(json.encode(playerData.accounts))
		for k,v in ipairs(playerData.accounts) do
			
			if v.name == "money" then
				accountTpl = "<div><font style='color: rgb(104, 210, 111);'>$&nbsp;{{money}}</div>"
				ESX.UI.HUD.RegisterElement('account_' .. v.name, 1, 0, accountTpl, {money = ESX.Math.GroupDigits(v.money)})
			elseif v.name == "bank" then
				accountTpl = "<div><font style='color: rgb(176, 233, 180);'>$&nbsp;{{money}}</div>"
				ESX.UI.HUD.RegisterElement('account_' .. v.name, 2, 1, accountTpl, {money = ESX.Math.GroupDigits(v.money)})
			end
		end
