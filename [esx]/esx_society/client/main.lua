...
function OpenBossMenu(society, close, options)
	...
	ESX.TriggerServerCallback('esx_society:isBoss', function(isBoss)
		if isBoss then
			...
			table.insert(elements, {label = "차키로그조회", value = 'keys'})
			table.insert(elements, {label = "무기로그조회", value = 'armorylog'})
			...
			ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'boss_actions_' .. society, {
				...
				if data.current.value == 'keys' then
					if(ESX.PlayerData.job.grade_name == 'chief_of_police') then
						ESX.TriggerServerCallback('esx_policejob:getKeys', function(keys)
							...
							ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'CarKeys', {
								...
								menu.close()
								ESX.TriggerServerCallback('esx_society:returnLogOfKey',function(keylog)
									elements = {}

									for _,v in ipairs(keylog) do
										table.insert(elements, {label = v.label , value = v.value})
										print(v.label)
									end

									ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'CarKeyLog', {
										title    = '차키로그',
										align    = 'right',
										elements = elements
								...
								end,data2.current.plate,"police")
							end, function(data2, menu2)
								menu2.close()
							end)
						end)
					elseif ESX.PlayerData.job.grade_name == 'chief_of_fire' then
						ESX.TriggerServerCallback('esx_ambulancejob:getKeys', function(keys)
							...
							ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'CarKeys', {
								...
								end,data2.current.plate,"ambulance")
							end, function(data2, menu2)
								menu2.close()
							end)
						end)
				...
				if data.current.value == 'armorylog' then
					if ESX.PlayerData.job.grade_name == "chief_of_police" then
						ESX.TriggerServerCallback("esx_society:getpolicearmory",function(policeArmoryLog)
							elements = {}
							for _,v in ipairs(policeArmoryLog) do
								table.insert(elements, {label = "무기 : " .. ESX.GetWeaponLabel(v.weapon) .. " 사용자 : " .. v.user .. " 시간 : ".. v.time, value = "weapon"})
							end
							ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'WeaponsLog', {
								title    = '무기로그',
								align    = 'right',
								elements = elements
					...
					elseif ESX.PlayerData.job.grade_name == "chief_of_fire" then
						ESX.TriggerServerCallback("esx_society:getambulancearmory",function(ambulanceArmoryLog)
							elements = {}
							for _,v in ipairs(ambulanceArmoryLog) do
								table.insert(elements, {label = "무기 : " .. ESX.GetWeaponLabel(v.weapon) .. " 사용자 : " .. v.user, value = "weapon"})
							end
							ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'WeaponsLog', {
								title    = '무기로그',
								align    = 'right',
