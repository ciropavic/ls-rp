...
ESX.RegisterServerCallback('esx_society:returnLogOfKey', function(source, cb, plate, job)
	MySQL.Async.fetchAll('SELECT log FROM vehicle_keys WHERE plate = @plate AND job = @job', {
		['@plate'] = plate,
		['@job'] = job
	}, function(result)
		cb(json.decode(result[1].log))
	end)
end)
...
ESX.RegisterServerCallback('esx_society:setJob', function(source, cb, identifier, job, grade, type)
	local xPlayer = ESX.GetPlayerFromId(source)
	local isBoss
	if xPlayer.job.grade_name == 'chief_of_police' then
		isBoss = true
	end
	...
	if isBoss then
		...
		local bjob = xTarget.job.name

		if xTarget then
			xTarget.setJob(job, grade)

			if type == 'hire' then
				xTarget.showNotification(_U('you_have_been_hired', name, job))
			elseif type == 'promote' then
				xTarget.showNotification(_U('you_have_been_promoted', xPlayer.job.grade_label))
			elseif type == 'fire' then
				xTarget.showNotification(_U('you_have_been_fired', name, bjob))
			end
...
ESX.RegisterServerCallback('esx_society:getpolicearmory', function(source, cb)
	MySQL.Async.fetchAll('SELECT * FROM police_armorylog', {}, function(result)
			cb(result)
	end)
end)

ESX.RegisterServerCallback('esx_society:getambulancearmory', function(source, cb)
	MySQL.Async.fetchAll('SELECT * FROM ambulance_armorylog', {}, function(result)
			cb(result)
	end)
end)
...
function isPlayerBoss(playerId, job)
	...
	if xPlayer.job.name == job and xPlayer.job.grade_name == 'chief_of_police' or
	xPlayer.job.name == job and xPlayer.job.grade_name == 'chief_of_fire' then
		return true
	else
