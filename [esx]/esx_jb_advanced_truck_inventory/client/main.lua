					...
					if ESX.UI.Menu.IsOpen('default', GetCurrentResourceName(), 'inventory') then
					  ...
					  if locked == 1 or class == 15 or class == 16 or class == 14 then
						  ...
						  TriggerServerEvent("esx_truck_inventory:getInventory", GetVehicleNumberPlateText(vehFront))
						   ...
						   ESX.ShowNotification('이 트렁크는 닫혀 있습니다.')
					  end
					end
				else
					ESX.ShowNotification('트렁크에 더 가까이 붙어주십시오.')
				end
				lastOpen = true
				GUI.Time  = GetGameTimer()
			else
				TriggerEvent('esx:showNotification', "누군가 이미 트렁크를 보고 있습니다.")
			end
		end, globalplate)
	end
...
local count = 0
RegisterCommand('차트렁크', function (source, args, rawCommand)
	openmenuvehicle()
end)
		  ...
		  if HasPedGotWeapon(playerPed,  weaponHash,  false) and weaponList[i].name ~= 'WEAPON_UNARMED' then
            ...
            if quantity > 0 and quantity <= tonumber(data3.current.count) and vehFront > 0  then
              ...
              --  VehicleMaxSpeed(closecar,totalweight,Config.VehicleLimit[GetVehicleClass(closecar)])
                ...
                ESX.ShowNotification('트렁크의 무게 : ~g~'.. Kgweight .. ' Kg / '..MaxVh..' Kg')
				Citizen.Wait(500)
				TriggerServerEvent("esx_truck_inventory:getInventory", GetVehicleNumberPlateText(vehFront))
              else
                ESX.ShowNotification('한도에 도달했습니다. ~r~ '..MaxVh..' Kg')
              end
			else
				ESX.ShowNotification('~r~ 잘못된 수량입니다.')
			end
		...
		elseif data.current.type == 'cancel' then
          ...
          --fin test
            ...
            if not max then
			   ...
			   local totalweight = tonumber(weight) - Itemweight
			   local Kgweight =  totalweight/1000
			   ESX.ShowNotification('상자 무게 : ~g~'.. Kgweight .. ' Kg / '..MaxVh..' Kg')
            else
              ESX.ShowNotification('당신은 너무 많이 착용했습니다.')
            end
			    else
			      ESX.ShowNotification('잘못된 수량입니다.')
			    end
