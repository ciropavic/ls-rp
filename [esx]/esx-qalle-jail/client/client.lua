...
AddEventHandler("esx-qalle-jail:jailPlayer", function(newJailTime)
	...
	print(random)
	if(random == 1) then
		JailPosition = Config.JailPositions["cell1"]
	end
	if(random == 2) then
		JailPosition = Config.JailPositions["cell2"]
	end
	if(random == 3) then
		JailPosition = Config.JailPositions["cell3"]
	end
...
function UnJail()
	...
	ESX.Game.Teleport(PlayerPedId(), Config.Release["Release"])
...
function OpenJailMenu()
	ESX.UI.Menu.Open(
		'default', GetCurrentResourceName(), 'jail_prison_menu',
		{
			...
			align    = 'center',
			elements = {
				{ label = "플레이어 수감", value = "jail_closest_player" },
				{ label = "플레이어 풀어주기", value = "unjail_player" }
			}
		}, 
	function(data, menu)
		...
		if action == "jail_closest_player" then
			...
			-- 해당좌표 변경
			if(GetDistanceBetweenCoords(462.69, -986.4, 34.2 , location,true) < 5) then
										...
										if reason == nil then
											...
											if closestPlayer == -1 or closestDistance > 3.0 then
												...
												if(GetPlayerServerId(closestPlayer) == Player) then
													TriggerServerEvent("esx-qalle-jail:jailPlayer", Player, jailTime, reason)
												else
													ESX.ShowNotification("수감 대상에게 더 가까이 가서 시도하십시오.")
												end
											end
				...
				ESX.ShowNotification("유치장 안에서 시도해 주십시오.")
			end
