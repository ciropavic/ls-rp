...
Citizen.CreateThread(function()
    while true do 
        ...
        if Vehicle.Vehicle ~= nil then
                ...
                if IsVehicleSeatFree(Vehicle.Vehicle, -1) and GetVehicleBodyHealth(Vehicle.Vehicle) <= 300.0 then
                    ESX.Game.Utils.DrawText3D({x = Vehicle.Coords.x, y = Vehicle.Coords.y, z = Vehicle.Coords.z}, ' [~g~SHIFT~w~] 키와 [~g~E~w~] 키를 동시에 눌러 차량을 밀 수 있습니다.', 0.4)
                end
     

            if IsControlPressed(0, Keys["LEFTSHIFT"]) and IsVehicleSeatFree(Vehicle.Vehicle, -1) and not IsEntityAttachedToEntity(ped, Vehicle.Vehicle) and IsControlJustPressed(0, Keys["E"])  and GetVehicleBodyHealth(Vehicle.Vehicle) <= 300.0 then
