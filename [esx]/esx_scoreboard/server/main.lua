...
RegisterServerEvent('esx_scoreboard:loadDB')

AddEventHandler('esx_scoreboard:loadDB', function()
...
function AddPlayerToScoreboard(xPlayer, update)
	...
	--print(MySQL.Async.fetchAll('SELECT * FROM users WHERE identifier = @identifier', { ['@identifier'] = xPlayer.identifier }))
	...
	connectedPlayers[playerId].ping = GetPlayerPing(playerId)
	connectedPlayers[playerId].id = playerId
	connectedPlayers[playerId].name = name
	print(connectedPlayers[playerId].name)
	connectedPlayers[playerId].job = xPlayer.job.name
