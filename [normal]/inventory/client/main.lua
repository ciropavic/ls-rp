...
RegisterNetEvent('inventory:closeditem')
AddEventHandler('inventory:closeditem',function()
    SendNUIMessage({
        action = 'close',
        invName = 'inventory'
    })
...
RegisterCommand('openInventory', function()
    ...
    if IsPedInAnyVehicle(PlayerPedId(), true) then
        ...
        for k,v in pairs(Drops) do
            local distance = #(v.coords - GetEntityCoords(PlayerPedId()))
            if distance < 2.0 then
                OpenInventory({id = k, type = 'drop', title = '🗑️ 바닥 (' .. k..')', coords = v.coords, weight = false, timeout = 1000})
                return
            end
        end 
        OpenInventory()
...
RegisterKeyMapping('OpenInventory', 'OpenInventory', 'keyboard', Config.DefaultOpenKey)

if Config.Hotbar then
    RegisterCommand('hotbar', function()
        ShowHotbar()
    end, true)
    RegisterKeyMapping('hotbar', 'ShowHotbar', 'keyboard', Config.HotbarKey) 
end