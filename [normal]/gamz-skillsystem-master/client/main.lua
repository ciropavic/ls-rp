...
Citizen.CreateThread(function()
	while true do
		...
		elseif IsPedInMeleeCombat(ped) then
			UpdateSkill("Strength", 1)
		elseif IsPedSwimmingUnderWater(ped) then
			UpdateSkill("Lung Capacity", 1)
		elseif IsPedShooting(ped) then
			UpdateSkill("Shooting", 1)
			...
			if GetVehicleClass(vehicle) == 8 or GetVehicleClass(vehicle) == 13 and speed >= 5 then
				...
				if IsControlPressed(0, 210) then
					if rotation.x >= 25.0 then
						UpdateSkill("Wheelie", 1)
					end 
				end
			end
			if speed >= 80 then
				UpdateSkill("Driving", 1)
			end
		end
	end
end)

AddEventHandler("base")

RegisterNetEvent("playerdied")
AddEventHandler("playerdied",function()
	UpdateSkill("Stamina", -20)
	UpdateSkill("Strength", -20)
	UpdateSkill("Lung Capacity", -20)
	UpdateSkill("Shooting", -20)
	UpdateSkill("Wheelie", -20)
	UpdateSkill("Driving", -20)
	print("skill reset")
end)