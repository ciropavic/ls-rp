...
function openSelectMenu(gang)
    Citizen.CreateThread(function()
        ...
        WarMenu.CreateSubMenu('members', 'opt', '이름 - 계급 - Steam')
        WarMenu.CreateSubMenu('info', 'opt', '팩션정보')
        WarMenu.CreateSubMenu('vehs', 'opt', '차량을 눌러서 삭제하세요.')
        WarMenu.CreateSubMenu('points', 'opt', '팩션 구역 변경')
        WarMenu.CreateSubMenu('shop', 'opt', '상점 항목 보기 및 제거')
        WarMenu.CreateSubMenu('addshop', 'opt', '상점에 아이템을 추가하겠습니까?')
        WarMenu.CreateSubMenu('delete', 'opt', '팩션을삭제하겠습니까?')
