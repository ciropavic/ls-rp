...
RegisterServerEvent("guille_gangs:server:invite")
AddEventHandler("guille_gangs:server:invite",function(args)
  local _src = source
    if isAllowed(_src) then
        local id = args[1]
        local rank = args[3]
        if args[1] and args[2] and args[3] then
          ...
          TriggerClientEvent("guille_gangs:client:okoknotify","ID,팩션이름,RANK 순으로 정확히 입력해주세요.",'error')
            -- TriggerClientEvent("guille_gangs:client:notify", _src, "Some arguments are missing")
        end
    else
      TriggerClientEvent("guille_gangs:client:okoknotify","당신은 사용할 권한이 없습니다.",'error')
    ...
    if tonumber(players[ply1.getSteam()].Player().getRank()) == tonumber(boss) then
        ...
        if args[1] and args[2]  then
            ...
            if not plys[ply.getSteam()] then
              TriggerClientEvent("guille_gangs:client:okoknotify","오류가 발생하였습니다. 해당 플레이어가 재접속해야 합니다.",'error') 
            ...
            if gangs[gang] ~= nil then
                if gangs[gang].gangInfo().isRankValid(rank) then
                  ...
                  TriggerClientEvent("guille_gangs:client:okoknotify","해당 계급은 존재하지 않습니다.",'error') 
                    -- TriggerClientEvent("guille_gangs:client:notify", _src, "That rank does not exist")
                end
            else
              TriggerClientEvent("guille_gangs:client:okoknotify","해당 팩션은 존재하지 않습니다.",'error') 
                -- TriggerClientEvent("guille_gangs:client:notify", _src, "That gang does not exist")
            end
        else
          TriggerClientEvent("guille_gangs:client:okoknotify","명령어를 다시 입력해주세요 Playerid, rank",'error') 
            -- TriggerClientEvent("guille_gangs:client:notify", _src, "Some arguments are missing")
        end
    else
      TriggerClientEvent("guille_gangs:client:okoknotify","당신은 사용할 권한이 없습니다.",'error') 
...
-- 계급 알기
RegisterServerEvent('guille_gangs:Sgettypes')
AddEventHandler('guille_gangs:Sgettypes',function(gang)
local rank = 0;
  MySQL.Async.fetchAll("SELECT * FROM guille_gangsv2 Where gang =@gang",{
    ['@gang'] = gang
  }, function (result)
    TriggerClientEvent('guille_gangs:Cgettypes',result.type)
  end)
