if not ESX then
	...
	local PlaySound = function(door, src)
		local origin
		if src and src ~= GetPlayerPed(-1) then src = NetworkGetEntityFromNetworkId(src) end
		if not src then origin = door.textCoords elseif src == GetPlayerPed(-1) then origin = playerCoords else origin = NetworkGetPlayerCoords(src) end
	...
	RegisterCommand('doorlock', function()
		if closestDoor.id and not ESX.PlayerData.dead and not isCuffed then
			...
			if closestDoor.data.audioRemote then src = NetworkGetNetworkIdFromEntity(GetPlayerPed(-1)) else src = nil end
	...
	local Raycast = function()
		local offset = GetOffsetFromEntityInWorldCoords(GetCurrentPedWeaponEntityIndex(GetPlayerPed(-1)), 0, 0, -0.01)
		...
		local rayHandle, result, hit, endCoords, surfaceNormal, entityHit = StartShapeTestLosProbe(offset, destination, -1, GetPlayerPed(-1), 0)
		repeat
			result, hit, endCoords, surfaceNormal, entityHit = GetShapeTestResult(rayHandle)
			Citizen.Wait(0)
		until result ~= 1
		print(GetCurrentPedWeaponEntityIndex(GetPlayerPed(-1)))
		print(GetSelectedPedWeapon(GetPlayerPed(-1)))
		print(hit,entityHit)
