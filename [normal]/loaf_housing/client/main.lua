...
Furnish = function(shell, data, houseid)
    if not Cache.furnishing then
        ...
        if Model.loaded then
            local object = CreateObject(Model.model, GetEntityCoords(PlayerPedId()), true, true, true)
            ...
            while Cache.furnishing do
                ...
                if IsControlPressed(0, 191) then
                    ...
                    if(Cache.houseid ~= nil and Cache.houseid ~= 0) then
                        DeleteEntity(object)
                    else
                        TriggerServerEvent("loaf_housing:deletefurnitureindb",data.object)
                    end
...
-- RegisterNetEvent("loaf_housing:placefurnitureoutside")
-- AddEventHandler("loaf_housing:placefurnitureoutside",function(model,position,object)
...
RegisterCommand("가구", function(source, args, rawCommand)
    print(Cache.houseid)
            ESX.UI.Menu.Open("default", GetCurrentResourceName(), "furnish", {
                ...
                if current2 == "put_furniture" then
                    ...
                    ESX.UI.Menu.Open("default", GetCurrentResourceName(), "select_furniture_put", {
                        ...
                        Furnish(Cache.shell_object, current3, tonumber(Cache.houseid))
                        ...
                        ESX.UI.Menu.Open("default", GetCurrentResourceName(), "remove_furniture_manage", {
                            ...
                            if current4 == "yes" then
                                ...
                                ESX.TriggerServerCallback("loaf_housing:removeFurniture", function(success)
                                    busy_spinner = false
                                end, tostring(Cache.houseid), current3.id)
                                ...
                                TriggerServerEvent("loaf_housing:removeFurniture", tostring(Cache.houseid), current3.id)
                            else
                                menu4.close()
                            end
