...
local screenPosY = 0.842                    -- Y coordinate (top left corner of HUD)
local screenPosY2 = 0.882
-- GENERAL PARAMETERS
...
local fuelColorOver = {46, 139, 87}       -- Color used to display fuel when good
...
Citizen.CreateThread(function()
    ...
    while true do
        ...
        if pedInVeh or locationAlwaysOn then
            ...
            drawTxt(timeText, 4, locationColorText, 0.4, screenPosX, screenPosY2 + 0.048)
            
            -- Display heading, street name and zone when possible
            drawTxt(locationText, 1, locationColorText, 0.5, screenPosX, screenPosY2 + 0.075)
            ...
            if pedInVeh and GetIsVehicleEngineRunning(vehicle) and vehicleClass ~= 13 then
                ...
                if IsControlJustReleased(0, seatbeltInput) and (enableController or GetLastInputMethod(0)) and vehicleClass ~= 8 then
                    ...
                    if seatbeltPlaySound then
                        if seatbeltIsOn then
                            exports['okokNotify']:Alert("시스템", "안전벨트를 맵니다.", 5000, 'success')
                        else
                            exports['okokNotify']:Alert("시스템", "안전벨트를 풉니다.", 5000, 'success')
                        end
                ...
                if ShouldUseMetricMeasurements() then
                    ...
                    drawTxt(("%.3d"):format(math.ceil(speed)), 2, speedColor, 0.8, screenPosX + 0.031, screenPosY - 0.035)
                    drawTxt("KPH", 4, speedColorText, 0.8, screenPosX + 0.000, screenPosY - 0.035)
                else
                    -- Get vehicle speed in MPH and draw speedometer
                    local speed = currSpeed*2.23694
                    local speedColor = (speed >= speedLimit) and speedColorOver or speedColorUnder
                    drawTxt(("%.3d"):format(math.ceil(speed)), 2, speedColor, 0.8, screenPosX + 0.030, screenPosY + 0.000)
                    drawTxt("MPH", 4, speedColorText, 0.6, screenPosX + 0.030, screenPosY + 0.018)
                end
                
                -- Draw fuel gauge
                local fuelColor = (currentFuel >= fuelWarnLimit) and fuelColorOver or fuelColorUnder
                drawTxt(("%.3d"):format(math.ceil(currentFuel)), 2, fuelColor, 0.75, screenPosX + 0.030, screenPosY + 0.005)
                drawTxt("%", 4, fuelColor, 0.75, screenPosX + 0.056, screenPosY + 0.005)
                drawTxt("FUEL", 4, fuelColorText, 0.75, screenPosX + 0.000, screenPosY + 0.005)

                -- Draw cruise control status
                local cruiseColor = cruiseIsOn and cruiseColorOn or cruiseColorOff
                drawTxt("CRUISE", 2, cruiseColor, 0.4, screenPosX + 0.04, screenPosY2 + 0.048)

                -- Draw seatbelt status if not a motorcyle
                if vehicleClass ~= 8 then
                    local seatbeltColor = seatbeltIsOn and seatbeltColorOn or seatbeltColorOff
                    drawTxt("SEATBELT", 2, seatbeltColor, 0.4, screenPosX + 0.080, screenPosY2 + 0.048)
                end
            end
        end
    end
    ...
    while true do
        ...
        if pedInVeh or locationAlwaysOn then
           ...
           -- local zoneNameFull = zones[GetNameOfZone(position.x, position.y, position.z)]
            ...
            locationText = (locationText) or (locationText)
