...
RegisterNUICallback('startJob', function(data, cb)
	if cooldown == nil then
		...
		if not loading then
			...
			TriggerEvent("Notify","importante",Lang[Config.lang]['loading_trailer'])
			closeUI()
		end
...
RegisterNUICallback('repairTruck', function(data, cb)
	if truck and not rentTruck then
		TriggerEvent("Notify","negado",Lang[Config.lang]['store_truck'])
		return
	end
...
RegisterNUICallback('sellTruck', function(data, cb)
	if cooldown == nil then
		...
		if truck and not rentTruck then
			TriggerEvent("Notify","negado",Lang[Config.lang]['store_truck_2'])
			return
		end
...
RegisterNUICallback('spawnTruck', function(data, cb)
	if cooldown == nil then
		...
		if not loading then
			...
			TriggerEvent("Notify","importante",Lang[Config.lang]['loading_truck'])
			closeUI()
		end
...
AddEventHandler('truck_logistics:spawnTruck', function(truck_data)
	...
	if truck then
		TriggerEvent("Notify","negado",Lang[Config.lang]['already_has_truck'])
		return
	end
...
AddEventHandler('truck_logistics:startContract', function(data,job_distance,reward,truck_data)
	...
	if trailer or rentTruck then
		TriggerEvent("Notify","negado",Lang[Config.lang]['already_has_cargo'])
		return
	end
	...
	if truck and data.contract_type == 0 then
		TriggerEvent("Notify","negado",Lang[Config.lang]['must_store_truck'])
		return
	end
	...
	TriggerEvent("Notify","sucesso",Lang[Config.lang]['started_job'])
	loading = false
	...
	while IsEntityAVehicle(trailer) do 
		...
		if bodyhealth <= 150 then
			PlaySoundFrontend(-1, "PROPERTY_PURCHASE", "HUD_AWARDS", 0)
			TriggerEvent("Notify","negado",Lang[Config.lang]['failed'])
...
function resetLoading()
	...
	if loading == true then
		TriggerEvent("Notify","negado",Lang[Config.lang]['loading_fail'])
		loading = false
	end
end
...
AddEventHandler('truck_logistics:vehicleClientLock', function()
	...
	if DoesEntityExist(v) and IsEntityAVehicle(v) then
		...
		if lock == 1 then
			...
			TriggerEvent("Notify","importante",Lang[Config.lang]['locked'],8000)
		else
			SetVehicleDoorsLocked(v,1)
			TriggerEvent("Notify","importante",Lang[Config.lang]['unlocked'],8000)
		end
