    ...
    while true do
        ...
        if canSearch then
            ...
            for i = 1, #dumpsters do
                ...
                if dist < 1.8 then
                    DrawText3Ds(dumpPos.x, dumpPos.y, dumpPos.z + 1.0, '[~y~H~w~] 키를 눌러 쓰레기를 수거하세요')
                    if IsControlJustReleased(0, 74) then
                        for i = 1, #searched do
                            ...
                            if i == #searched and dumpsterFound then
                                exports['mythic_notify']:DoHudText('error', '누군가에 의해 이미 수거된 쓰레기통입니다..')
                            elseif i == #searched and not dumpsterFound then
                                exports['mythic_notify']:DoHudText('inform', '쓰레기통을 뒤적거리기 시작합니다.')
...
function startSearching(time, dict, anim, cb)
    ...
    exports.rprogress:Start("garbage", time)
