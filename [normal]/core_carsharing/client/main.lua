...
function spawnCar(x, y, z, h)
    ESX.TriggerServerCallback("core_carsharing:check_license", function(result) 
        if result then
            ...
            local model = Config.Vehicles[math.random(4, #Config.Vehicles)]
            local car = CreateVehicle(GetHashKey(model.Model), x, y, z, h, true, true)
            SetVehicleDoorsLocked(car, 2)
            SetVehicleNumberPlateText(car, Config.vehiclePlateText)
            SetVehicleCustomPrimaryColour(car, 255, 255, 255)
...
RegisterNetEvent("core_carsharing:autopay")
AddEventHandler("")
Citizen.CreateThread(
    function()
        while true do
            ...
            if currentveh == current then
                ...
                if currentHealth < lastDamage - (1000 * (Config.damagePercentForPenalty / 100)) then
                    ...
                    exports['okokNotify']:Alert("시스템",string.gsub(Config.Text['damagePenalty'], '{penaltyPrice}', Config.damagePenalty), 5000, 'info')
                    -- SendTextMessage(string.gsub(Config.Text['damagePenalty'], '{penaltyPrice}', Config.damagePenalty))
                end
            elseif current ~= currentveh then
                if current ~= nil then
                    vehicleoutTime = vehicleoutTime + 1
                    if vehicleoutTime == 540 then
                        exports['okokNotify']:Alert("시스템","렌트카 반납까지 1분 남았습니다.", 5000, 'info')
                    end
                    
                    -- if Config.penaltyOutsideZone > 0 and inZone() == false and vehicleoutTime == 60 then
                    --     TriggerServerEvent("core_carsharing:pay", Config.penaltyOutsideZone)
                    --     exports['okokNotify']:Alert("시스템",string.gsub(Config.Text['penaltyPaid'], '{penaltyPrice}', Config.penaltyOutsideZone), 5000, 'info')
                    --     -- SendTextMessage(string.gsub(Config.Text['penaltyPaid'], '{penaltyPrice}', Config.penaltyOutsideZone))
                    -- end

                    -- if Config.badParkingDelete and inZone() == false and vehicleoutTime == 60 then
                    --     exports['okokNotify']:Alert("시스템","렌트카가 자동반납되었습니다.", 5000, 'info')
                    ...
                    if paid == false then
                        ...
                        exports['okokNotify']:Alert("시스템",string.gsub(Config.Text['paid'], '{price}', tostring(price)), 5000, 'info')
            ...
            for _, i in ipairs(vehicles) do
                ...
                if currentveh == i.vehicle then
                    if current == nil and GetPedInVehicleSeat(currentveh, -1) == GetPlayerPed(-1) then
                        current = i.vehicle
                        TriggerServerEvent("core_carsharing:createvehicle",current)
                                    ...
                                    ESX.TriggerServerCallback(
                                        ...
                                        function(hasMoney)
                                            if hasMoney then
                                                ...
                                                exports['okokNotify']:Alert("시스템",Config.Text['vehicleUnlocked'], 5000, 'info')
                                                -- SendTextMessage(Config.Text['vehicleUnlocked'])
                                                
                                            else
                                                -- local vehicleModel = GetEntityModel(i.vehicle)
                                                -- print(vehicleModel)
                                                exports['okokNotify']:Alert("시스템",Config.Text['noMoneyOrLicense'], 5000, 'error')
                                                -- SendTextMessage(Config.Text['noMoneyOrLicense'])
                                            end
                                        end,
                                        Config.vehicleUnlockFee,
                                        GetEntityModel(i.vehicle)
                                    )
                                end
                            end
                        end
