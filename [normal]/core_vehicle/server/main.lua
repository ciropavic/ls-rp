...
ESX.RegisterServerCallback("core_vehicle:returnLevel",function(source, cb)
	local xPlayer = ESX.GetPlayerFromId(source)
	MySQL.Async.fetchAll('SELECT mechaniclevel FROM users WHERE identifier = @identifier', {
        ['@identifier'] = xPlayer.identifier
    }, function(result)
        cb(result[1].mechaniclevel)
    end)
end)

RegisterServerEvent('core_vehicle:exp')
AddEventHandler('core_vehicle:exp', function()
	local xPlayer = ESX.GetPlayerFromId(source)
	MySQL.Async.fetchAll('SELECT mechanicexp FROM users WHERE identifier = @identifier', {
        ['@identifier'] = xPlayer.identifier
    }, function(resultexp)
        MySQL.Async.fetchAll('SELECT mechaniclevel FROM users WHERE identifier = @identifier', {
			['@identifier'] = xPlayer.identifier
		}, function(resultlevel)
			if(resultlevel[1].mechaniclevel == 1) then
				if resultexp[1].mechanicexp == 100 then
					...
					MySQL.Sync.execute("UPDATE `users` SET `mechanicexp`=@mechanicexp WHERE identifier = @identifier", { 
						['@mechanicexp'] = resultexp[1].mechanicexp + 1,
						['@identifier'] = xPlayer.identifier
					})
				end
			elseif resultlevel[1].mechaniclevel == 2 then
				if resultexp[1].mechanicexp == 200 then
					MySQL.Sync.execute("UPDATE `users` SET `mechaniclevel`=@mechaniclevel WHERE identifier = @identifier", { 
						['@mechaniclevel'] = 3,
						['@identifier'] = xPlayer.identifier
					})
			...
			elseif resultlevel[1].mechaniclevel == 3 then
				if resultexp[1].mechanicexp == 300 then
					MySQL.Sync.execute("UPDATE `users` SET `mechaniclevel`=@mechaniclevel WHERE identifier = @identifier", { 
						['@mechaniclevel'] = 4,
						['@identifier'] = xPlayer.identifier
					})
			...
			elseif resultlevel[1].mechaniclevel == 4 then
				if resultexp[1].mechanicexp == 400 then
					MySQL.Sync.execute("UPDATE `users` SET `mechaniclevel`=@mechaniclevel WHERE identifier = @identifier", { 
						['@mechaniclevel'] = 5,
						['@identifier'] = xPlayer.identifier
					})
